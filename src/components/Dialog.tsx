...
function DialogContent({
  ...
  const handleInteractOutside =
    onInteractOutside || (preventOutsideClick ? (e: Event) => e.preventDefault() : undefined);

  return (
    <DialogPortal data-slot="dialog-portal">
      <DialogOverlay data-testid={`${testId}-overlay`} />
      ...
      >
        ...
        {showCloseButton && (
          <DialogPrimitive.Close
            ...
            className="absolute top-4 right-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-gray-100 data-[state=open]:text-gray-500"
          >
            <XIcon />
...
function DialogFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={`flex flex-col-reverse gap-2 sm:flex-row sm:justify-end ${className || ""} pt-8`}
      {...props}
    />
  );
}
